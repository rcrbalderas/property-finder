...
<script setup>
    ...
    const paginate = async (pageNumber = 1) => {
        ...
        const {posts: fetchedPosts, total} = await paginateProperties(page);
        //console.log('properties:', fetchedPosts);
        posts.value = await fetchedPosts;
    ...
    const handlePageClick = async (pageNumber) => {
        await router.push({ path: '/properties', query: { page: pageNumber } } , { shallow: true });
